...
function agentBadgeText(agentId: string, defaultId: string | null) {
  return defaultId && agentId === defaultId ? t("agents.default", "default") : null;
}
...
function buildAgentContext(
  ...
  return {
    ...
    skillsLabel: skillFilter
      ? `${skillCount} ${t("agents.selected", "selected")}`
      : t("agents.allSkills", "all skills"),
...
function buildModelOptions(configForm: Record<string, unknown> | null, current?: string | null) {
  ...
  if (current && !hasCurrent) {
    options.unshift({ value: current, label: `${t("agents.current", "Current")} (${current})` });
  }
  if (options.length === 0) {
    return html`
      <option value="" disabled>${t("agents.noConfiguredModels", "No configured models")}</option>
    `;
  }
    ...
    <div class="agents-layout">
      <section class="card agents-sidebar">
        <div class="row" style="justify-content: space-between;">
          <div>
            <div class="card-title">${t("agents.title", "Agents")}</div>
            <div class="card-sub">${agents.length} ${t("agents.configured", "configured.")}</div>
          </div>
          <button class="btn btn--sm" ?disabled=${props.loading} @click=${props.onRefresh}>
            ${props.loading ? t("agents.loading", "Loading…") : t("agents.refresh", "Refresh")}
          </button>
        </div>
        ${
          props.error
        ...
        <div class="agent-list" style="margin-top: 12px;">
          ${
            agents.length === 0
              ? html`
                  <div class="muted">${t("agents.noAgents", "No agents found.")}</div>
                `
              : agents.map((agent) => {
      ...
      <section class="agents-main">
        ${
          !selectedAgent
            ? html`
                <div class="card">
                  <div class="card-title">${t("agents.selectAgent", "Select an agent")}</div>
                  <div class="card-sub">${t("agents.selectAgent.sub", "Pick an agent to inspect its workspace and tools.")}</div>
                </div>
              `
            : html`
              ${renderAgentHeader(
...
function renderAgentHeader(
  ...
  const subtitle =
    agent.identity?.theme?.trim() || t("agents.headerSub", "Agent workspace and routing.");
...
function renderAgentTabs(active: AgentsPanel, onSelect: (panel: AgentsPanel) => void) {
  const tabs: Array<{ id: AgentsPanel; label: string }> = [
    { id: "overview", label: t("agents.overview", "Overview") },
    { id: "files", label: t("agents.coreFiles", "Core Files") },
    { id: "tools", label: t("agents.toolAccess", "Tool Access") },
    { id: "skills", label: t("agents.skills", "Skills") },
    { id: "channels", label: t("agents.channels", "Channels") },
    { id: "cron", label: t("agents.cronJobs", "Cron Jobs") },
  ];
  return html`
...
function renderAgentOverview(params: {
  ...
  const identityStatus = agentIdentityLoading
    ? t("agents.loading", "Loading…")
    : agentIdentityError
      ? t("agents.unavailable", "Unavailable")
      : "";
  const isDefault = Boolean(params.defaultId && agent.id === params.defaultId);

  return html`
    <section class="card">
      <div class="card-title">${t("agents.overview", "Overview")}</div>
      <div class="card-sub">${t("agents.overview.sub", "Workspace paths and identity metadata.")}</div>
      <div class="agents-overview-grid" style="margin-top: 16px;">
        <div class="agent-kv">
          <div class="label">${t("agents.workspace", "Workspace")}</div>
          <div class="mono">${workspace}</div>
        </div>
        <div class="agent-kv">
          <div class="label">${t("agents.primaryModel", "Primary Model")}</div>
          <div class="mono">${model}</div>
        </div>
        <div class="agent-kv">
          <div class="label">${t("agents.identityName", "Identity Name")}</div>
          <div>${identityName}</div>
          ${identityStatus ? html`<div class="agent-kv-sub muted">${identityStatus}</div>` : nothing}
        </div>
        <div class="agent-kv">
          <div class="label">${t("agents.default", "Default")}</div>
          <div>${isDefault ? t("agents.yes", "yes") : t("agents.no", "no")}</div>
        </div>
        <div class="agent-kv">
          <div class="label">${t("agents.identityEmoji", "Identity Emoji")}</div>
          <div>${identityEmoji}</div>
        </div>
        <div class="agent-kv">
          <div class="label">${t("agents.skillsFilter", "Skills Filter")}</div>
          <div>${skillFilter ? `${skillCount} ${t("agents.selected", "selected")}` : t("agents.allSkills", "all skills")}</div>
        </div>
      </div>

      <div class="agent-model-select" style="margin-top: 20px;">
        <div class="label">${t("agents.modelSelection", "Model Selection")}</div>
        <div class="row" style="gap: 12px; flex-wrap: wrap;">
          <label class="field" style="min-width: 260px; flex: 1;">
            <span>${t("agents.primaryModelLabel", "Primary model")}${isDefault ? ` (${t("agents.default", "default")})` : ""}</span>
            <select
                ...
                isDefault
                  ...
                  : html`
                      <option value="">
                        ${
                          defaultPrimary
                            ? `${t("agents.inheritDefault", "Inherit default")} (${defaultPrimary})`
                            : t("agents.inheritDefault", "Inherit default")
                        }
                      </option>
                    `
              }
          ...
          <label class="field" style="min-width: 260px; flex: 1;">
            <span>${t("agents.fallbacks", "Fallbacks (comma-separated)")}</span>
            <input
              .value=${fallbackText}
              ?disabled=${!configForm || configLoading || configSaving}
        ...
        <div class="row" style="justify-content: flex-end; gap: 8px;">
          ...
          >
            ${t("agents.reloadConfig", "Reload Config")}
          </button>
          <button
            ...
            @click=${onConfigSave}
          >
            ${configSaving ? t("agents.saving", "Saving…") : t("agents.save", "Save")}
          </button>
        </div>
      </div>
    </section>
...
function renderAgentContextCard(context: AgentContext, subtitle: string) {
  return html`
    <section class="card">
      <div class="card-title">${t("agents.agentContext", "Agent Context")}</div>
      <div class="card-sub">${subtitle}</div>
      <div class="agents-overview-grid" style="margin-top: 16px;">
        <div class="agent-kv">
          <div class="label">${t("agents.workspace", "Workspace")}</div>
        ...
        <div class="agent-kv">
          <div class="label">${t("agents.default", "Default")}</div>
          <div>${context.isDefault ? t("agents.yes", "yes") : t("agents.no", "no")}</div>
        </div>
      </div>
    </section>
  `;
...
function renderAgentChannels(params: {
  ...
  return html`
    <section class="grid grid-cols-2">
      ${renderAgentContextCard(context, t("agents.workspaceTargets", "Workspace, identity, and model configuration."))}
      <section class="card">
        <div class="row" style="justify-content: space-between;">
          <div>
            <div class="card-title">${t("agents.channels", "Channels")}</div>
            <div class="card-sub">${t("agents.channels.sub", "Gateway-wide channel status snapshot.")}</div>
          </div>
          <button class="btn btn--sm" ?disabled=${params.loading} @click=${params.onRefresh}>
            ${params.loading ? t("agents.refreshing", "Refreshing…") : t("agents.refresh", "Refresh")}
          </button>
        </div>
        <div class="muted" style="margin-top: 8px;">
          ${t("agents.lastRefresh", "Last refresh")}: ${lastSuccessLabel}
        </div>
        ${
          params.error
        ...
        ${
          !params.snapshot
            ? html`
                <div class="callout info" style="margin-top: 12px">${t("agents.loadChannels", "Load channels to see live status.")}</div>
              `
            : nothing
        }
        ${
          entries.length === 0
            ? html`
                <div class="muted" style="margin-top: 16px">${t("agents.noChannels", "No channels found.")}</div>
              `
            : html`
              <div class="list" style="margin-top: 16px;">
                ${entries.map((entry) => {
                  ...
                  const status = summary.total
                    ? `${summary.connected}/${summary.total} ${t("agents.channel.connected", "connected")}`
                    : t("agents.channel.noAccounts", "no accounts");
                  const config = summary.configured
                    ? `${summary.configured} ${t("agents.channel.configured", "configured")}`
                    : t("agents.channel.notConfigured", "not configured");
                  const enabled = summary.total
                    ? `${summary.enabled} ${t("agents.channel.enabled", "enabled")}`
                    : t("agents.channel.disabled", "disabled");
...
function renderAgentCron(params: {
  ...
  return html`
    <section class="grid grid-cols-2">
      ${renderAgentContextCard(context, t("agents.workspaceTargets", "Workspace and scheduling targets."))}
      <section class="card">
        <div class="row" style="justify-content: space-between;">
          <div>
            <div class="card-title">${t("agents.scheduler", "Scheduler")}</div>
            <div class="card-sub">${t("agents.schedulerStatus", "Gateway cron status.")}</div>
          </div>
          <button class="btn btn--sm" ?disabled=${params.loading} @click=${params.onRefresh}>
            ${params.loading ? t("agents.refreshing", "Refreshing…") : t("agents.refresh", "Refresh")}
          </button>
        ...
        <div class="stat-grid" style="margin-top: 16px;">
          <div class="stat">
            <div class="stat-label">${t("agents.enabled", "Enabled")}</div>
            <div class="stat-value">
              ${params.status ? (params.status.enabled ? t("agents.yes", "Yes") : t("agents.no", "No")) : "n/a"}
            </div>
          </div>
          <div class="stat">
            <div class="stat-label">${t("agents.jobs", "Jobs")}</div>
            <div class="stat-value">${params.status?.jobs ?? "n/a"}</div>
          </div>
          <div class="stat">
            <div class="stat-label">${t("agents.nextWake", "Next wake")}</div>
    ...
    <section class="card">
      <div class="card-title">${t("agents.cronJobs", "Cron Jobs")}</div>
      <div class="card-sub">${t("agents.cronJobs.sub", "Scheduled jobs targeting this agent.")}</div>
      ${
        jobs.length === 0
          ? html`
              <div class="muted" style="margin-top: 16px">${t("agents.noJobs", "No jobs assigned.")}</div>
            `
          : html`
              <div class="list" style="margin-top: 16px;">
                ${jobs.map(
                    ...
                    <div class="list-main">
                      ...
                      <div class="chip-row" style="margin-top: 6px;">
                        ...
                        <span class="chip ${job.enabled ? "chip-ok" : "chip-warn"}">
                          ${job.enabled ? t("agents.enabled.lower", "enabled") : t("agents.disabled.lower", "disabled")}
                        </span>
...
function renderAgentFiles(params: {
  ...
  return html`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          <div class="card-title">${t("agents.coreFiles", "Core Files")}</div>
          <div class="card-sub">${t("agents.coreFiles.sub", "Bootstrap persona, identity, and tool guidance.")}</div>
        </div>
        <button
          class="btn btn--sm"
        ...
        >
          ${params.agentFilesLoading ? t("agents.loading", "Loading…") : t("agents.refresh", "Refresh")}
        </button>
      </div>
      ${list ? html`<div class="muted mono" style="margin-top: 8px;">${t("agents.workspace", "Workspace")}: ${list.workspace}</div>` : nothing}
      ${
        ...
        !list
          ? html`
              <div class="callout info" style="margin-top: 12px">
                ${t("agents.loadFiles", "Load the agent workspace files to edit core instructions.")}
              </div>
            `
          : html`
              <div class="agent-files-grid" style="margin-top: 16px;">
                <div class="agent-files-list">
                  ${
                    files.length === 0
                      ? html`
                          <div class="muted">${t("agents.noFiles", "No files found.")}</div>
                        `
                      : files.map((file) =>
                ...
                <div class="agent-files-editor">
                  ${
                    !activeEntry
                      ? html`
                          <div class="muted">${t("agents.selectFile", "Select a file to edit.")}</div>
                        `
                      : html`
                          <div class="agent-file-header">
                            ...
                            <div class="agent-file-actions">
                              ...
                              >
                                ${params.agentFileSaving ? t("agents.saving", "Saving…") : t("agents.save", "Save")}
                              </button>
                            </div>
                          </div>
                          ${
                            activeEntry.missing
                              ? html`
                                  <div class="callout info" style="margin-top: 10px">
                                    ${t("agents.fileMissing", "This file is missing. Saving will create it in the agent workspace.")}
                                  </div>
                                `
                              : nothing
                          }
                          <label class="field" style="margin-top: 12px;">
                            <span>${t("agents.content", "Content")}</span>
                            <textarea
                              .value=${draft}
                              @input=${(e: Event) =>
  ...
  return html`
    ...
    >
      ...
      ${
        file.missing
          ? html`
              <span class="agent-pill warn">${t("agents.missing.lower", "missing")}</span>
            `
          : nothing
      }
    </button>
...
function renderAgentTools(params: {
  ...
  const profileSource = agentTools.profile
    ? t("agents.agentOverride", "agent override")
    : globalTools.profile
      ? t("agents.globalDefault", "global default")
  ...
  return html`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          <div class="card-title">${t("agents.toolAccess", "Tool Access")}</div>
          <div class="card-sub">
            ${t("agents.toolAccess.sub", "Profile + per-tool overrides for this agent.")}
            <span class="mono">${enabledCount}/${toolIds.length}</span> ${t("agents.enabled.lower", "enabled")}.
          </div>
        </div>
        <div class="row" style="gap: 8px;">
          ...
          >
            ${t("agents.disableAll", "Disable All")}
          </button>
          <button
            class="btn btn--sm"
          ...
          >
            ${params.configSaving ? t("agents.saving", "Saving…") : t("agents.save", "Save")}
          </button>
        </div>
      </div>
      ...
      ${
        !params.configForm
          ? html`
              <div class="callout info" style="margin-top: 12px">
                ${t("agents.loadConfig", "Load the gateway config to adjust tool profiles.")}
              </div>
            `
          : nothing
      }
      ${
        hasAgentAllow
          ? html`
              <div class="callout info" style="margin-top: 12px">
                ${t("agents.explicitAllowlist", "This agent is using an explicit allowlist in config. Tool overrides are managed in the Config tab.")}
              </div>
            `
          : nothing
      }
      ${
        hasGlobalAllow
          ? html`
              <div class="callout info" style="margin-top: 12px">
                ${t("agents.globalAllow", "Global tools.allow is set. Agent overrides cannot enable tools that are globally blocked.")}
              </div>
            `
          : nothing
      }
      ...
      <div class="agent-tools-meta" style="margin-top: 16px;">
        <div class="agent-kv">
          <div class="label">${t("agents.profile", "Profile")}</div>
          <div class="mono">${profile}</div>
        </div>
        <div class="agent-kv">
          <div class="label">${t("agents.source", "Source")}</div>
        ...
        ${
          params.configDirty
            ? html`
                <div class="agent-kv">
                  <div class="label">${t("agents.status", "Status")}</div>
                  <div class="mono">${t("agents.unsaved", "unsaved")}</div>
                </div>
              `
            : nothing
        }
      ...
      <div class="agent-tools-presets" style="margin-top: 16px;">
        <div class="label">${t("agents.quickPresets", "Quick Presets")}</div>
        <div class="agent-tools-buttons">
          ${PROFILE_OPTIONS.map(
            (option) => html`
              ...
              >
                ${t("agents.profile." + option.id, option.label)}
              </button>
            `,
          )}
          <button
        ...
        ${TOOL_SECTIONS.map(
          (section) =>
            ...
            <div class="agent-tools-section">
              <div class="agent-tools-header">${t("agents.tools.section." + section.id, section.label)}</div>
                  ...
                  return html`
                    <div class="agent-tool-row">
                      <div>
                        ...
                        <div class="agent-tool-sub">${t("agents.tools." + tool.id + ".desc", tool.description)}</div>
                      </div>
...
function renderAgentSkills(params: {
  ...
  return html`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          <div class="card-title">${t("agents.skills", "Skills")}</div>
          <div class="card-sub">
            ${t("agents.skills.sub", "Per-agent skill allowlist and workspace skills.")}
        ...
        <div class="row" style="gap: 8px;">
          ...
          <button class="btn btn--sm" ?disabled=${params.loading} @click=${params.onRefresh}>
            ${params.loading ? t("agents.loading", "Loading…") : t("agents.refresh", "Refresh")}
          </button>
          <button
            class="btn btn--sm primary"
            ?disabled=${params.configSaving || !params.configDirty}
      ...
      ${
        !params.configForm
          ? html`
              <div class="callout info" style="margin-top: 12px">
                ${t("agents.loadGatewayConfig", "Load the gateway config to set per-agent skills.")}
              </div>
            `
          : nothing
      }
      ${
        usingAllowlist
          ? html`
              <div class="callout info" style="margin-top: 12px">${t("agents.customAllowlist", "This agent uses a custom skill allowlist.")}</div>
            `
          : html`
              <div class="callout info" style="margin-top: 12px">
                ${t("agents.allSkillsEnabled", "All skills are enabled. Disabling any skill will create a per-agent allowlist.")}
              </div>
            `
      }
      ${
        !reportReady && !params.loading
          ? html`
              <div class="callout info" style="margin-top: 12px">
                ${t("agents.loadSkills", "Load skills for this agent to view workspace-specific entries.")}
              </div>
            `
          : nothing
      }
      ...
      <div class="filters" style="margin-top: 14px;">
        <label class="field" style="flex: 1;">
          <span>${t("agents.filter", "Filter")}</span>
          <input
            .value=${params.filter}
            @input=${(e: Event) => params.onFilterChange((e.target as HTMLInputElement).value)}
            placeholder=${t("agents.searchSkills", "Search skills")}
          />
        </label>
        <div class="muted">${filtered.length} ${t("agents.shown", "shown")}</div>
      </div>

      ${
        filtered.length === 0
          ? html`
              <div class="muted" style="margin-top: 16px">${t("agents.noSkills", "No skills found.")}</div>
            `
          : html`
  ...
  return html`
    <details class="agent-skills-group" ?open=${!collapsedByDefault}>
      <summary class="agent-skills-header">
        <span>${t("agents.skillGroup." + group.id, group.label)}</span>
...
function renderAgentSkillRow(
  ...
  if (skill.disabled) {
    reasons.push(t("agents.skill.disabled", "disabled"));
  }
  if (skill.blockedByAllowlist) {
    reasons.push(t("agents.skill.blockedByAllowlist", "blocked by allowlist"));
  }
  return html`
      ...
      <div class="list-main">
        ...
        <div class="chip-row" style="margin-top: 6px;">
          ...
          <span class="chip ${skill.eligible ? "chip-ok" : "chip-warn"}">
            ${skill.eligible ? t("agents.skill.eligible", "eligible") : t("agents.skill.blocked", "blocked")}
          </span>
          ${
            skill.disabled
              ? html`
                  <span class="chip chip-warn">${t("agents.skill.disabled", "disabled")}</span>
                `
              : nothing
          }
        </div>
        ${
          missing.length > 0
            ? html`<div class="muted" style="margin-top: 6px;">${t("agents.skill.missing", "Missing")}: ${missing.join(", ")}</div>`
            : nothing
        }
        ${
          reasons.length > 0
            ? html`<div class="muted" style="margin-top: 6px;">${t("agents.skill.reason", "Reason")}: ${reasons.join(", ")}</div>`
            : nothing
        }
      </div>
      <div class="list-meta">
