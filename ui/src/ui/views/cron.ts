...
export function renderCron(props: CronProps) {
  ...
  const selectedRunTitle =
    selectedJob?.name ?? props.runsJobId ?? t("cron.selectJob", "(select a job)");
  ...
  return html`
      ...
      <div class="card">
        ...
        <div class="row" style="margin-top: 12px;">
          <button class="btn" ?disabled=${props.loading} @click=${props.onRefresh}>
            ${props.loading ? t("cron.refreshing", "Refreshing…") : t("cron.refresh", "Refresh")}
          </button>
      ...
      <div class="card">
        ...
        <div class="form-grid" style="margin-top: 16px;">
          ...
          <label class="field">
            ...
            >
              <option value="every">${t("cron.every", "Every")}</option>
              <option value="at">${t("cron.at", "At")}</option>
              <option value="cron">${t("cron.cronExpr", "Cron")}</option>
            </select>
          </label>
        </div>
        ...
        <div class="form-grid" style="margin-top: 12px;">
          <label class="field">
            ...
            >
              <option value="main">${t("cron.main", "Main")}</option>
              <option value="isolated">${t("cron.isolated", "Isolated")}</option>
            </select>
          </label>
          <label class="field">
            ...
            >
              <option value="now">${t("cron.now", "Now")}</option>
              <option value="next-heartbeat">${t("cron.nextHeartbeat", "Next heartbeat")}</option>
            </select>
          </label>
          <label class="field">
            ...
            >
              <option value="systemEvent">${t("cron.systemEvent", "System event")}</option>
              <option value="agentTurn">${t("cron.agentTurn", "Agent turn")}</option>
            </select>
          </label>
        </div>
        ...
        ${
          props.form.payloadKind === "agentTurn"
            ? html`
                <div class="form-grid" style="margin-top: 12px;">
                  <label class="field">
                    <span>${t("cron.deliver", "Delivery")}</span>
                    <select
                    ...
                    >
                      <option value="announce">${t("cron.announce", "Announce summary (default)")}</option>
                      <option value="none">${t("cron.noneInternal", "None (internal)")}</option>
                    </select>
                  </label>
                  <label class="field">
    ...
    <section class="card" style="margin-top: 18px;">
      ...
      ${
        props.jobs.length === 0
          ? html`
              <div class="muted" style="margin-top: 12px">${t("cron.noJobs", "No jobs yet.")}</div>
            `
          : html`
    ...
    <section class="card" style="margin-top: 18px;">
      ...
      <div class="card-sub">${t("cron.runs.sub", "Latest runs for")} ${selectedRunTitle}.</div>
      ${
        props.runsJobId == null
          ? html`
              <div class="muted" style="margin-top: 12px">${t("cron.selectJob", "Select a job to inspect run history.")}</div>
            `
          : orderedRuns.length === 0
            ? html`
                <div class="muted" style="margin-top: 12px">${t("cron.noRuns", "No runs yet.")}</div>
              `
            : html`
              <div class="list" style="margin-top: 12px;">
  ...
  return html`
    <div class=${itemClass} @click=${() => props.onLoadRuns(job.id)}>
      <div class="list-main">
        ...
        ${job.agentId ? html`<div class="muted cron-job-agent">${t("cron.agent", "Agent")}: ${job.agentId}</div>` : nothing}
      </div>
      <div class="list-meta">
        ${renderJobState(job)}
      </div>
      <div class="cron-job-footer">
        <div class="chip-row cron-job-chips">
          <span class=${`chip ${job.enabled ? "chip-ok" : "chip-danger"}`}>
            ${job.enabled ? t("cron.enabled", "enabled") : t("cron.disabled", "disabled")}
          </span>
...
function renderJobPayload(job: CronJob) {
  if (job.payload.kind === "systemEvent") {
    return html`<div class="cron-job-detail">
      <span class="cron-job-detail-label">${t("cron.system", "System")}</span>
  ...
  return html`
    <div class="cron-job-detail">
      <span class="cron-job-detail-label">${t("cron.prompt", "Prompt")}</span>
      <span class="muted cron-job-detail-value">${job.payload.message}</span>
    </div>
    ${
      delivery
        ? html`<div class="cron-job-detail">
            <span class="cron-job-detail-label">${t("cron.deliver", "Delivery")}</span>
  ...
  return html`
    <div class="cron-job-state">
      <div class="cron-job-state-row">
        <span class="cron-job-state-key">${t("cron.status", "Status")}</span>
        <span class=${`cron-job-status-pill ${statusClass}`}>${status}</span>
      </div>
      <div class="cron-job-state-row">
        <span class="cron-job-state-key">${t("cron.next", "Next")}</span>
        <span class="cron-job-state-value" title=${formatMs(nextRunAtMs)}>
          ${formatStateRelative(nextRunAtMs)}
        </span>
      </div>
      <div class="cron-job-state-row">
        <span class="cron-job-state-key">${t("cron.last", "Last")}</span>
  ...
  return html`
    <div class="list-item">
      ...
      <div class="list-meta">
        ...
        ${
          chatUrl
            ? html`<div><a class="session-link" href=${chatUrl}>${t("cron.openRunChat", "Open run chat")}</a></div>`
            : nothing
        }
