...
export function renderNostrCard(params: {
      ...
      <div class="account-card">
        ...
        <div class="status-list account-card-status">
          <div>
            ...
            <span>${account.running ? t("channels.yes", "Yes") : t("channels.no", "No")}</span>
          </div>
          <div>
            <span class="label">${t("channels.configured", "Configured")}</span>
            <span>${account.configured ? t("channels.yes", "Yes") : t("channels.no", "No")}</span>
          </div>
          <div>
            <span class="label">${t("channels.nostr.publicKey", "Public Key")}</span>
            <span class="monospace" title="${publicKey ?? ""}">${truncatePubkey(publicKey)}</span>
          </div>
          <div>
            <span class="label">${t("channels.lastInbound", "Last inbound")}</span>
            <span>${account.lastInboundAt ? formatRelativeTimestamp(account.lastInboundAt) : t("channels.na", "n/a")}</span>
          </div>
          ${
            account.lastError
              ? html`
    ...
    return html`
      <div style="margin-top: 16px; padding: 12px; background: var(--bg-secondary); border-radius: 8px;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
          <div style="font-weight: 500;">${t("channels.nostr.profile", "Profile")}</div>
          ${
            summaryConfigured
              ? html`
                <button
                ...
                >
                  ${t("channels.nostr.editProfile", "Edit Profile")}
                </button>
              `
              : nothing
          }
          ...
          hasAnyProfileData
            ? html`
              <div class="status-list">
                  ...
                  picture
                    ? html`
                      <div style="margin-bottom: 8px;">
                        <img
                          src=${picture}
                          alt="${t("channels.nostr.profilePicture", "Profile picture")}"
                ...
                ${name ? html`<div><span class="label">${t("channels.nostr.name", "Name")}</span><span>${name}</span></div>` : nothing}
                ${
                  displayName
                    ? html`<div><span class="label">${t("channels.nostr.displayName", "Display Name")}</span><span>${displayName}</span></div>`
                    : nothing
                }
                ${
                  about
                    ? html`<div><span class="label">${t("channels.nostr.about", "About")}</span><span style="max-width: 300px; overflow: hidden; text-overflow: ellipsis;">${about}</span></div>`
                    : nothing
                }
            ...
            : html`
                <div style="color: var(--text-muted); font-size: 13px">
                  ${t("channels.nostr.noProfile", 'No profile set. Click "Edit Profile" to add your name, bio, and avatar.')}
                </div>
              `
        }
      </div>
  ...
  return html`
    <div class="card">
      ...
      <div class="card-sub">${t("channels.nostr.sub", "Decentralized DMs via Nostr relays (NIP-04).")}</div>
      ${accountCountLabel}
        ...
        hasMultipleAccounts
          ...
          : html`
            <div class="status-list" style="margin-top: 16px;">
              <div>
                <span class="label">${t("channels.configured", "Configured")}</span>
                <span>${summaryConfigured ? t("channels.yes", "Yes") : t("channels.no", "No")}</span>
              </div>
              <div>
                <span class="label">${t("channels.running", "Running")}</span>
                <span>${summaryRunning ? t("channels.yes", "Yes") : t("channels.no", "No")}</span>
              </div>
              <div>
                <span class="label">${t("channels.nostr.publicKey", "Public Key")}</span>
              ...
              <div>
                ...
                <span>${summaryLastStartAt ? formatRelativeTimestamp(summaryLastStartAt) : t("channels.na", "n/a")}</span>
              </div>
            </div>
          `
      }
      ...
      <div class="row" style="margin-top: 12px;">
        <button class="btn" @click=${() => props.onRefresh(false)}>${t("channels.refresh", "Refresh")}</button>
      </div>
    </div>
  `;
}
