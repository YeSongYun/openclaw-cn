...
export async function statusAllCommand(
  ...
  await withProgress(
    { label: tst("scanAll.label", "Scanning status --all…"), total: 11 },
    async (progress) => {
      ...
      progress.setLabel(tst("scanAll.services", "Checking services…"));
      ...
      progress.setLabel(tst("scanAll.scanningAgents", "Scanning agents…"));
      ...
      progress.setLabel(tst("scanAll.queryGateway", "Querying gateway…"));
      ...
      progress.setLabel(tst("scanAll.localState", "Checking local state…"));
